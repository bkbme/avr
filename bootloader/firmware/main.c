...
int main(void)
{
    ...
    if(bootLoaderCondition()){
        ...
        do{ /* main event loop */
            ...
            /* Check if system is currently programming.
             * If not and the main loop is not being exited at the moment
             * sleep 100us every iteration for 255*255 iternation.
             * Effectivly, the system is waiting for about 6s to be programmed
             * over usb after reset and then boots the application.
             */
            if(!isProgramming && !exitMainloop)
            {
              _delay_us(100);
              if(--h==0)
              {
